 ...
 * early termination (when |x| > bound, backpropagate win or loss), and robust child. Instead of using the same value
 * for alpha and exploration the entire search, the alpha value decreases when the number of visits increases with a
 * specified slope, while the exploration value gets determined with a softmax (with temperature) based on the implicit
 * minimax values.
 */
public class ImplicitMCTSNNMSP extends MCTS {
