...
public class UBFMNNCompletedTraining extends NNBot {
     ...
     */
    public UBFMNNCompletedTraining(MultiLayerNetwork net) {
    ...
    /**
     * Selects and returns an action to play based on completed UBFM. The search algorithm evaluates all children
        ...
        if (context.trial().over()) {
            ...
            if (tableData == null || tableData.resolution == 0){
                ...
                if (sortedCompletedMoves == null) {
                    ...
                    if (depth == 0 || this.dataSelection == Enums.DataSelection.TREE){
                        inputNN = this.leafEvaluator.boardToInput(context);
                        this.TTTraining.store(zobrist, completedMove.score, depth, inputNN.data().asFloat());
                    }
                } else {
