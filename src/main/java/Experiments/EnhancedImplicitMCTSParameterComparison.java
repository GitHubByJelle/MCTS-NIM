...
/**
 * Main class which can be used to run tournaments of enhanced implicit MCTS bots with different parameters to fine-tune
 * the parameters. It will run a tournament for all given alpha, C and slope combinations.
 */
public class EnhancedImplicitMCTSParameterComparison {
    /**
     * Main class which runs a tournament of a single bot pair. It will run a tournament for
     * all given alpha, C and slope combinations. The assumption is made that the first agent is an enhanced implicit MCTS
     ...
     *             double[] alpha: influence of estimated value of implicit MCTS (multiple values can be given when separated with ",")
     *             double[] C: Exploitation constant (multiple values can be given when separated with ",")
     *             double[] slope: Slopes for alpha decrease (multiple values can be given when separated with ",")
     *             double[] minAlpha: Minimum bound of influence of estimated value of implicit MCTS (multiple values can be given when separated with ",")
     * @throws IOException
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if (args.length == 0) {
            ...
            props = propertyLoader.getPropertyValues("configurations\\templateEnhancedImplicitMCTS.properties");
        } else {
        ...
        final double[] explorationConstant = Stream.of(props.getProperty("C").split(","))
                .mapToDouble(Double::parseDouble).toArray();
        final double[] slope = Stream.of(props.getProperty("slope").split(","))
                .mapToDouble(Double::parseDouble).toArray();
        final double[] minAlpha = Stream.of(props.getProperty("minAlpha").split(","))
                .mapToDouble(Double::parseDouble).toArray();
        ...
        testParamValues(agentClassOne, agentClassTwo, agentOneNN, alpha, explorationConstant, slope, minAlpha,
    ...
    public static void testParamValues(final String agentClassOne, final String agentClassTwo,
                                       ...
                                       double[] minAlpha, String agentTwoNN, String gameName, final int numGames,
    ...
    public static void testParamMultipleGames(final int side, final String agentClassOne, final String agentClassTwo,
                                              ...
                                              final double slope, final double minAlpha, String agentTwoNN, String gameName,
        ...
        IntStream.range(0, numGames).forEach((i) -> {
            int winner = testParamSingleGame(side, agentClassOne, agentClassTwo, agentOneNN, alpha, explorationConstant,
                    slope, minAlpha, agentTwoNN, gameName, maxSeconds, maxIterations, maxDepth);
    ...
    public static int testParamSingleGame(final int side, final String agentClassOne,
                                          ...
                                          final double explorationConstant, final double slope, final double minAlpha,
