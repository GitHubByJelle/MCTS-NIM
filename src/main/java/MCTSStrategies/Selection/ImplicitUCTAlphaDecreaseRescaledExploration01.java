 ...
 * exploration. The MCTS score (winrate) and heuristic values are scaled between 0 and 1 (instead of -1 and 1)
 */
public class ImplicitUCTAlphaDecreaseRescaledExploration01 extends ImplicitUCT {
     ...
     */
    public ImplicitUCTAlphaDecreaseRescaledExploration01(double initialInfluenceEstimatedMinimax, double explorationConstant,
    ...
    public int select(MCTS mcts, BaseNode current) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            estimatedValues[i] = (estimatedValues[i] + 1) / 2;
        }
        ...
        for (int i = 0; i < numChildren; ++i) {
            implicitNode child = (implicitNode) current.childForNthLegalMove(i);
            if (child == null) {
                exploit = (unvisitedValueEstimate + 1) / 2;
                explore = explorationProbs[i] * Math.sqrt(parentLog);
                numVisits = 0;
            } else {
                exploit = (child.exploitationScore(moverAgent) + 1) / 2;
