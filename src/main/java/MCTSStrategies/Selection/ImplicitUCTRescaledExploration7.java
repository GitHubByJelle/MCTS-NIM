 ...
 * However, instead of multiplying all children with the same exploration constant, a softmax with temperature 1/n_c
 * is used to transform the estimated values to probabilities which are multiplied with the exploration. For this
 * implementation, the ln(n_p) in the exploration is removed (inspired by AlphaZero)
 */
public class ImplicitUCTRescaledExploration7 extends ImplicitUCT {
     ...
     */
    public ImplicitUCTRescaledExploration7(double influenceEstimatedMinimax, Softmax rescaler) {
     ...
     * softmax with temperature 1/n_c on the estimated values, and the ln(n_p) in the exploration formula
     * is removed (inspired by AlphaZero).
     *
